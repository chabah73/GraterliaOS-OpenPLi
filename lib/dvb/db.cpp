...
void eDVBDB::saveServicelist(const char *file)
{
			...
			i != m_channels.end(); ++i)
	{
		...
		sprintf(myLinetmp, "%08x;%04x;%04x", chid.dvbnamespace.get() ,chid.transport_stream_id.get(), chid.original_network_id.get());
		myLine = myLinetmp; //encryptDecrypt(myLinetmp);
		fprintf(f, "%s\n", myLine.c_str() );
		eDVBFrontendParametersSatellite sat;
		eDVBFrontendParametersTerrestrial ter;
		eDVBFrontendParametersCable cab;
		...
		if (!ch.m_frontendParameters->getDVBS(sat))
		{
			...
			{
				sprintf(myLinetmp, "\ts %d;%d;%d;%d;%d;%d;%d",
					sat.frequency, sat.symbol_rate,
					sat.polarisation, sat.fec,
					sat.orbital_position > 1800 ? sat.orbital_position - 3600 : sat.orbital_position,
					sat.inversion, flags);
